/****************************************************************************
** $Id: helpwindow.cpp,v 1.1 2003/05/02 14:32:07 dschmidt Exp $
**
      ...
      pathCombo( 0 ), selectedURL()
{
    ...
    file->insertItem( tr("E&xit"), qApp, SLOT( closeAllWindows() ), CTRL+Key_X );
        ...
        QPixmap backmap(back_xpm);
        QPixmap forwardmap(forward_xpm);
        QPixmap homemap(home_xpm);
        
    // The same three icons are used twice each.
    QIconSet icon_back( backmap );
    QIconSet icon_forward( forwardmap );
    QIconSet icon_home( homemap );
...
void HelpWindow::textChanged()
{
    if ( browser->documentTitle().isNull() )
        setCaption( "mdiswiz - Help - " + browser->context() );
    else
        setCaption( "mdiswiz - Help - " + browser->documentTitle() ) ;
...
HelpWindow::~HelpWindow()
{
    ...
    QFile f( QDir::currentDirPath() + "/.helphistory" );
    f.open( IO_WriteOnly );
    QDataStream s( &f );
    s << history;
    f.close();
    ...
    QFile f2( QDir::currentDirPath() + "/.helpbookmarks" );
...
void HelpWindow::print()
{
    ...
    if ( printer.setup( this ) ) {
        ...
        QFont font( "times", 10 );
        QSimpleRichText richText( browser->text(), font,
                                  browser->context(), 
                                  browser->styleSheet(),
...
{
    if ( QFile::exists( QDir::currentDirPath() + "/.helphistory" ) ) {
        QFile f( QDir::currentDirPath() + "/.helphistory" );
        f.open( IO_ReadOnly );
        QDataStream s( &f );
        s >> history;
...
{
    if ( QFile::exists( QDir::currentDirPath() + "/.helpbookmarks" ) ) {
        QFile f( QDir::currentDirPath() + "/.helpbookmarks" );
        f.open( IO_ReadOnly );
        QDataStream s( &f );
        s >> bookmarks;
        f.close();
